...
TORCH_CONV = nn.modules.conv.Conv2d
TORCH_BATCHNORM = nn.modules.batchnorm._BatchNorm
TORCH_PRELU = nn.PReLU
TORCH_LINEAR = nn.Linear
TORCH_CONVTRANS = nn.modules.conv._ConvTransposeNd
TORCH_DECONV = torchvision.ops.DeformConv2d
...
def _get_module_type(module):
    if isinstance(module, TORCH_CONV):
        ...
        return OPTYPE.SPLIT
    elif isinstance(module, TORCH_CONVTRANS):
        return OPTYPE.CONVTRANS
    elif isinstance(module, TORCH_DECONV):
        return OPTYPE.DECONV
    else:
        return OPTYPE.ELEMENTWISE


def _get_node_out_channel(node):
    if node.type == OPTYPE.CONV or node.type == OPTYPE.GROUP_CONV or node.type == OPTYPE.DECONV or node.type == OPTYPE.CONVTRANS:
...
class PruningPlan(object):
    ...
    def exec(self, dry_run=False):
        ...
        for i, (dep, idxs) in enumerate(self._plans):
            if 'related' not in dep.handler.__name__ and 'conv_offset_mask' in dep.broken_node.node_name:
                continue
            if any(['conv_offset_mask' in node.node_name for node in dep.broken_node.inputs]):
                continue
